...
private static final long serialVersionUID = 4902928151136583142L;
...
*/
  public JIniFile(File userFileName) {
    clear();
  ...
  public ArrayList<String> ReadSections() {
    ...
    for (int i=0; i < size(); i++) {
      ...
      if (s.startsWith("[") && s.endsWith("]")) {
        list.add(s.substring(1,(s.length()-1)).trim());
      }
    }
    return list;
  }
    ...
    if (this.SectionPosition(Section) > -1) {
      for (int i=start; i < size(); i++) {
        ...
        if (s.startsWith("[") && s.endsWith("]")) {
          ...
          myList.add(s.substring(0, s.indexOf("=")).trim());
        }
      }
    }
    return myList;
